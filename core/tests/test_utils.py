...
def test_model_supports_function_calling():
    ...
    # LINEAJE: enforce() `_lineaje_payload` at agent->policy_engine security_decision — scan flagged AI_APP_SEC_006 (Use only LLMs from the organization's approved list.); AI_APP_SEC_028 (Do not use LLMs from the organization's disallowed list). Mask/block; do not remove without review. site_id='site:sha256:44f0f24f1c71fa3ea3ae751526f90f0857fdffd8f96bbf6489e7cd45742e9d48'
    _gr_client = _lineaje_load_gr_client()
    _gr_site = _gr_client.SiteDescriptor(site_id='site:sha256:44f0f24f1c71fa3ea3ae751526f90f0857fdffd8f96bbf6489e7cd45742e9d48', phase='security_decision', boundary={'source': 'agent_message', 'sink': 'agent_message'}, candidate_policies=[], fail_mode='ALLOW_WITH_AUDIT', source_type='agent', destination_type='policy_engine')
...
def test_parse_chunk_response_func_calling(chunks_stream_answer):
    ...
    for chunk in chunks_stream_answer:
        ...
        # LINEAJE: enforce() `rolling_msg` at agent->policy_engine security_decision — scan flagged AI_APP_SEC_023 (Client must validate and sanitize any output from a MCP server); AI_APP_SEC_029 (Agent must validate, sanitize LLM output including for presence of eval or any dynamic code execution primitive in LLM output.). Mask/block; do not remove without review. site_id='site:sha256:581c59a085c6fa7448ca06adb0c7e1158c5840a48be345ee727a135ad5e2e241'
        _gr_client = _lineaje_load_gr_client()
        _gr_site = _gr_client.SiteDescriptor(site_id='site:sha256:581c59a085c6fa7448ca06adb0c7e1158c5840a48be345ee727a135ad5e2e241', phase='security_decision', boundary={'source': 'agent_message', 'sink': 'agent_message'}, candidate_policies=[], fail_mode='ALLOW_WITH_AUDIT', source_type='agent', destination_type='policy_engine')
        rolling_msg = _gr_client.enforce(_gr_site, rolling_msg, content_type='application/json', variable_name='rolling_msg', source_file=__file__, before_line=71)
