...
def test_model_supports_function_calling():
    ...
    # LINEAJE: enforce() `_lineaje_payload` at agent->policy_engine security_decision — scan flagged AI_APP_SEC_006 (Use only LLMs from the organization's approved list.); AI_APP_SEC_028 (Do not use LLMs from the organization's disallowed list). Mask/block; do not remove without review. site_id='site:sha256:7756008d1ba6bf1bf3f33e76fb5eadf27f395e91bf36b2b6626d4c32c32eccd0'
    _gr_client = _lineaje_load_gr_client()
    _gr_site = _gr_client.SiteDescriptor(site_id='site:sha256:7756008d1ba6bf1bf3f33e76fb5eadf27f395e91bf36b2b6626d4c32c32eccd0', phase='security_decision', boundary={'source': 'agent_message', 'sink': 'agent_message'}, candidate_policies=[], fail_mode='ALLOW_WITH_AUDIT', source_type='agent', destination_type='policy_engine')
...
def test_parse_chunk_response_func_calling(chunks_stream_answer):
    ...
    for chunk in chunks_stream_answer:
        ...
        # LINEAJE: enforce() `rolling_msg` at agent->policy_engine security_decision — scan flagged AI_APP_SEC_029 (Agent must validate, sanitize LLM output including for presence of eval or any dynamic code execution primitive in LLM output.). Mask/block; do not remove without review. site_id='site:sha256:7fdce77cc528b9997c0dd8f0cc5cad170311e10e538c0037a7fc95cec049571d'
        _gr_client = _lineaje_load_gr_client()
        _gr_site = _gr_client.SiteDescriptor(site_id='site:sha256:7fdce77cc528b9997c0dd8f0cc5cad170311e10e538c0037a7fc95cec049571d', phase='security_decision', boundary={'source': 'agent_message', 'sink': 'agent_message'}, candidate_policies=[], fail_mode='ALLOW_WITH_AUDIT', source_type='agent', destination_type='policy_engine')
